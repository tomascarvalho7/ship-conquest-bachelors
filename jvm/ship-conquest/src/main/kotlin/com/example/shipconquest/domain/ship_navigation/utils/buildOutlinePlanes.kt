...
private fun buildPlaneFn(a: Vector2, b: Vector2, thickness: Double, previousReference: Vector2?, nextReference: Vector2?): Plane {
    ...
    val normal = a.getNormalBetween(b).normalize() * thickness
    ...
    // combine them
    val unitNormalA = normal.plus(previousNormal).normalize()
    val unitNormalB = normal.plus(nextNormal).normalize()
...
private fun Vector2.getNormalBetween(other: Vector2): Vector =
    Vector(x = (other.x - x).toDouble(), y = (-other.y + y).toDouble())